///*
//	virtual function : 파생클래스에서 재정의할 것을 약속받은 멤버 함수를 말하며 
//	Base 클래스의 멤버 함수에 virutal 키워드를 사용하여 만든다.
...
//	bptr->func2();							// base클래스의 func1()은 가상함수로 선언되었지만 오버라이딩(재정의)이 되지않았다.
//	bptr->func3();							// 포인터 타입의 호출 함수가 결정된다.(가상함수x)
//	
//	delete bptr;
//	
//	bptr = &b;
...
//// 이것은 컴파일 시 포인터형으로 바인딩되지 못하고 런타임에서 포인터가 어떤 객체를 가리키고 있는지에 따라서 바인딩할 함수를 결정한다.